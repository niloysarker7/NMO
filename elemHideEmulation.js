...
function scopedQuerySelector(subtree, selector, all) {
  if (selector[0] == '>') {
    ...
    if (scopeSupported == null) return tryQuerySelector(subtree, selector, all);
    return null;
  }
  return all
    ? subtree.querySelectorAll(selector)
...
class NotSelector {
  ...
  *getElements(prefix, subtree, targets) {
    let actualPrefix =
      !prefix || incompletePrefixRegexp.test(prefix) ? prefix + '*' : prefix;
    ...
    if (elements) {
      for (let element of elements) {
        ...
        if (!this._innerPattern.matches(element, subtree)) yield element;
    ...
    if (elements) {
      for (let element of elements) {
        ...
        for (let selector of this._innerPattern.evaluate(element, targets)) {
          ...
          else if (scopedQuerySelector(element, selector)) yield element;
        }
...
class PropsSelector {
  ...
  *getSelectors(prefix, subtree, targets) {
    for (let subSelector of this._subSelectors) {
      if (subSelector.startsWith('*') && !incompletePrefixRegexp.test(prefix))
...
class Pattern {
  ...
  get maybeDependsOnAttributes() {
    ...
    return getCachedPropertyValue(this, '_maybeDependsOnAttributes', () =>
      this.selectors.some(
        (selector) =>
          selector.maybeDependsOnAttributes ||
          (selector instanceof HasSelector && selector.dependsOnStyles)
      )
    );
  }
...
function extractMutationTargets(mutations) {
  ...
  for (let mutation of mutations) {
    if (mutation.type == 'childList') {
      ...
      if (mutation.removedNodes.length > 0) targets.add(mutation.target);
    } else {
...
exports.ElemHideEmulation = class ElemHideEmulation {
  ...
  async _addSelectors(stylesheets, mutations) {
    ...
    if (shouldObserveStyles(this.patterns)) this._refreshPatternStyles();
    ...
    for (let pattern of patternsToCheck) {
      ...
      if (pattern.maybeContainsSiblingCombinators) evaluationTargets = null;
  ...
  _unhideElems(elementsToUnhide) {
    if (elementsToUnhide.length > 0) {
      ...
      for (let elem of elementsToUnhide) this.hiddenElements.delete(elem);
    }
  }
