      ...
      c.exports = function (c) {
        ...
        function q(c) {
          var M = 0,
            ...
            p = 0,
            n = 1;
          for (45 === c.charCodeAt(0) && ((M = 1), (n = -1)); M < A.length; M++)
            p = 60 * p + O(A.charCodeAt(M));
          for (M = 0; M < o.length; M++)
            (b /= 60), (p += O(o.charCodeAt(M)) * b);
          return p * n;
        }
        function L(c) {
          for (var M = 0; M < c.length; M++) c[M] = q(c[M]);
        }
        function N(c, M) {
          var z,
            A = [];
        ...
        function X(c, M) {
          for (var z, A; (A = 6e4 * (((M.at - c.at) / 12e4) | 0)); )
            (z = new e(new Date(c.at + A))).offset === c.offset
              ? (c = z)
              : (M = z);
          return c;
        }
        ...
        function B(c, M) {
          var z, A;
          for (L(M), z = 0; z < M.length; z++)
            (A = M[z]), (b[A] = b[A] || {}), (b[A][c] = !0);
        }
        function r(c) {
          var M,
            z,
            ...
            a = [];
          for (M = 0; M < p; M++)
            for (z in (A = b[c[M].offset] || {}))
              A.hasOwnProperty(z) && (n[z] = !0);
          for (M in n) n.hasOwnProperty(M) && a.push(o[M]);
          return a;
        }
        function T() {
          try {
          ...
          var z,
            ...
            p = (function () {
              var c,
                ...
                o = new e(new Date(A, 0, 1)),
                b = [o];
              for (z = 1; z < 48; z++)
                (M = new e(new Date(A, z, 1))).offset !== o.offset &&
                  ((c = X(o, M)),
                  b.push(c),
                  b.push(new e(new Date(c.at + 6e4)))),
                  (o = M);
              for (z = 0; z < 4; z++)
                b.push(new e(new Date(A + z, 0, 1))),
                  b.push(new e(new Date(A + z, 6, 1)));
              return b;
            })(),
            n = p.length,
            a = r(p),
          ...
          for (A = 0; A < a.length; A++) {
            for (z = new d(t(a[A]), n), b = 0; b < n; b++)
              z.scoreOffsetAt(p[b]);
            O.push(z);
          }
          return O.sort(f), O.length > 0 ? O[0].zone.name : void 0;
        }
        function l(c) {
        ...
        function t(c, M) {
          ...
          return p instanceof W
            ...
            ? ((p = new W(p)), (z[c] = p), p)
            : A[c] && M !== t && (b = t(A[c], t))
            ? ((p = z[c] = new W())._set(b), (p.name = o[c]), p)
            : null;
        }
        function s(c) {
          var M, z, b, p;
        ...
        function C(c) {
          u(c.zones), s(c.links), (S.dataVersion = c.version);
        }
        function E(c) {
          var M = 'X' === c._f || 'x' === c._f;
        ...
        function S(M) {
          var z = Array.prototype.slice.call(arguments, 0, -1),
            ...
            o = t(A),
            b = c.utc.apply(null, z);
          return (
            o && !c.isMoment(M) && E(b) && b.add(o.parse(b), 'minutes'),
            b.tz(A),
            b
          );
        }
        (n < 2 || (2 === n && a < 6)) &&
            ...
            parse: function (c) {
              ...
              for (o = 0; o < a; o++)
                if (
                  ...
                  (z = p[o + 1]),
                  (A = p[o ? o - 1 : o]),
                  M < z && S.moveAmbiguousForward
                    ? (M = z)
                    : M > A && S.moveInvalidForward && (M = A),
                  b < n[o] - 6e4 * M)
                )
                  return p[o];
              return p[a];
          ...
          (d.prototype.scoreOffsetAt = function (c) {
            (this.offsetScore += Math.abs(
              this.zone.utcOffset(c.at) - c.offset
            )),
          ...
          (S.names = function () {
            var c,
              M = [];
            for (c in o)
              o.hasOwnProperty(c) && (z[c] || z[A[c]]) && o[c] && M.push(o[c]);
            return M.sort();
          }),
          (S.Zone = W),
          (S.unpack = i),
        ...
        (c.tz = S),
            ...
            if (
              (void 0 === M._z &&
                (o &&
                  E(M) &&
                  !M._isUTC &&
                  ((M._d = c.utc(M._a)._d), M.utc().add(o.parse(M), 'minutes')),
                (M._z = o)),
              M._z)
            )
              if (
          ...
          (D.zoneName = g(D.zoneName)),
          (D.zoneAbbr = g(D.zoneAbbr)),
          (D.utc = P(D.utc)),
          (D.local = P(D.local)),
          (D.utcOffset =
            ((m = D.utcOffset),
          ...
          (c.tz.setDefault = function (M) {
            return (
              ...
              (c.defaultZone = M ? t(M) : null),
              c
            );
          });
        var h = c.momentProperties;
        return (
