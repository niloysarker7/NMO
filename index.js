...
import { ElemHideEmulation } from 'adblockpluscore/lib/content/elemHideEmulation.js';
...
async function initContentFeatures() {
  ...
  if (response.emulatedPatterns.length > 0) {
    let elemHideEmulation = new ElemHideEmulation(
      ...
      (elements) => {
        for (let element of elements) unhideElement(element);
      }
    );
    elemHideEmulation.apply(response.emulatedPatterns);
  }

  if (response.subscribeLinks) handleSubscribeLinks();
}
