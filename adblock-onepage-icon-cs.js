...
(function onScriptLoad() {
  ...
  function loadIconResources(base, callback) {
    function loadCss(cssSrc) {
      ...
      const fontCSSPromise = fetch(fontCssUrl).then((response) =>
        response.text()
      );
    ...
    function loadFont(name, style, weight, unicodeRange) {
      return new FontFace(
        'Lato',
        `url(${browser.runtime.getURL(`/fonts/${name}.woff`)}`,
        { style, weight, unicodeRange }
      );
    }
    ...
    fonts.push(
      new FontFace(
        'Material Icons',
        `url(${browser.runtime.getURL('/icons/MaterialIcons-Regular.woff2')}`,
    ...
    fonts.push(
      new FontFace(
        'AdBlock Icons',
        `url(${browser.runtime.getURL('/icons/adblock-icons.woff2')}`,
        { style: 'normal', weight: 'normal' }
      )
    );
    Promise.all(fonts)
    ...
    if (mainBody) {
            ...
            <div class="grow speech-bubble">
              <div id="header-icons">
                <span  class="header-section">
                  <img class="header-logo" alt="AdBlock logo" src="${browser.runtime.getURL(
      ...
      if (DOMPurify.removed && DOMPurify.removed.length > 0) {
        browser.runtime.sendMessage({
          ...
          msg: 'onpagemessage_invalid_msg',
          additionalParams: { surveyId },
        });
        return;
      }
      ...
      settingsIcon.onclick = function settingsClicked(event) {
        ...
        browser.runtime.sendMessage({
          command: 'recordGeneralMessage',
          msg: 'onpagemessage_settings_clicked',
          additionalParams: { surveyId },
        });
      };
      ...
      closeIcon.onclick = function closedClicked(event) {
        ...
        browser.runtime.sendMessage({
          command: 'recordGeneralMessage',
          msg: 'onpagemessage_closed_clicked',
          additionalParams: { surveyId },
        });
      };
      ...
      if (buttonText && buttonAction) {
        leanMoreBtn.onclick = function learnMoreClicked(event) {
          ...
          if (buttonAction && buttonAction.startsWith('/')) {
            browser.runtime.sendMessage({
              command: 'openTab',
              urlToOpen: `https://getadblock.com${buttonAction}`,
            });
          } else if (buttonAction && buttonAction.startsWith('#')) {
            browser.runtime.sendMessage({
              command: 'openTab',
              urlToOpen: browser.runtime.getURL(`options.html${buttonAction}`),
            });
          }
          browser.runtime.sendMessage({
            command: 'recordGeneralMessage',
            msg: 'onpagemessage_btn_clicked',
            additionalParams: { surveyId },
          });
        };
      } else {
      ...
      const determineOnPageIconBottomPostion = () => {
        ...
        if (host === 'www.facebook.com') {
          // Special processing for FB
          const newMsgIconNodeList = document.querySelectorAll(
            problemSites[host].selector
          );
        ...
        const conflictingElement = document.querySelector(
          problemSites[host].selector
        );
      ...
      loadIconResources(baseShadow, async () => {
        ...
        baseShadow.host.style.setProperty(
  ...
  browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (
      request.command === 'showonpageicon' &&
      request.tabURL === document.location.href
    ) {
      showOverlay(
        request.titlePrefixText,
        request.titleText,
        request.msgText,
        request.buttonText,
        request.ctaIconURL,
