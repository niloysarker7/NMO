...
const splitMessageWithReplacementText = function (rawMessageText, messageID) {
  ...
  returnObj.anchorText = rawMessageText.substring(
    anchorStartPos + 2,
    anchorEndPos
  );
...
const processReplacementChildren = function ($el, replacementText, messageId) {
  ...
  if ($(replaceElId).length === 0) {
    log(
      'returning, no child element found',
      $element.attr('i18n'),
      replaceElId
    );
    return;
  ...
  const messageSplit = splitMessageWithReplacementText(
    rawMessageText,
    messageID
  );
  ...
  if ($(replaceElId).get(0).tagName === 'INPUT') {
    ...
    $(`#${$element.attr('i18n_replacement_el')}`).text(
      replacementText || messageSplit.anchorText
    );
  }
...
const isLangRTL = function (language) {
  ...
  return (
    lang.startsWith('ar') || lang.startsWith('he') || lang.startsWith('fa')
  );
};
...
const localizePage = function () {
  ...
  $('[i18n_replacement_el]:not(.i18n-replaced)').each(
    function i18nReplacementEl() {
...
parseUri.secondLevelDomainOnly = function stripThirdPlusLevelDomain(
  domain,
  keepDot
) {
  if (domain) {
    const match = domain.match(/([^.]+\.(?:co\.)?[^.]+)\.?$/) || [
      domain,
      domain,
    ];
...
const ellipsis = function ellipsis(valueToTruncate, maxSize) {
  ...
  if (value.length > size) {
    value = `${value.substring(0, half)}...${value.substring(
      value.length - half
    )}`;
  }
